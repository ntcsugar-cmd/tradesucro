...
export function LivePriceTable({ entries, loading = false }: LivePriceTableProps) {
  const columns: DataTableColumn<MillPriceEntry>[] = [
    ...
    { key: "previousPrice", header: "Previous Price", render: (e) => <span className="font-mono text-ink-faint dark:text-white/40">{formatINR(e.previousPrice)}</span> },
    {
      key: "change",
      header: "Change",
      render: (e) => {
        ...
        return (
          <span className={`flex items-center gap-1 font-mono text-xs ${diff > 0 ? "text-rise" : diff < 0 ? "text-fall" : "text-ink-faint dark:text-white/40"}`}>
    ...
    { key: "offerAvailable", header: "Offer", render: (e) => (e.offerAvailable ? <Check size={14} className="text-success" /> : <X size={14} className="text-ink-faint dark:text-white/40" />) },
    { key: "tenderOpen", header: "Tender", render: (e) => (e.tenderOpen ? <Check size={14} className="text-success" /> : <X size={14} className="text-ink-faint dark:text-white/40" />) },
