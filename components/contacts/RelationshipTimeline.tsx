      ...
      {events.map((e, i) => {
        ...
        return (
          <li key={e.id} className="pl-5 relative">
            <span className={`absolute -left-[13px] top-0.5 flex h-6 w-6 items-center justify-center rounded-full ${i === 0 ? "bg-gold/15 text-gold-dim" : "bg-charcoal/[0.05] text-ink-faint"}`}>
              <Icon size={12} />
            </span>
            <p className="text-[13px] font-medium text-charcoal">{e.description}</p>
            <p className="text-xs text-ink-faint mt-0.5">{timeAgo(e.timestamp)}</p>
          </li>
        );
      })}
    </ol>
      ...
      {events.map((e) => {
        ...
        return (
          <li key={e.id} className="flex items-start gap-3 py-3 first:pt-0 last:pb-0">
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-charcoal/[0.05] text-ink-faint">
              <Icon size={14} />
            </span>
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-ink-faint mt-0.5">{timeAgo(e.timestamp)}</p>
            </div>
          </li>
        );
      })}
