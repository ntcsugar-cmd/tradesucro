...
export function TraderSubNav() {
  ...
  return (
    <div className="sticky top-0 z-dropdown -mx-6 mb-8 border-b border-line bg-paper/95 px-6 backdrop-blur-sm md:-mx-10 md:px-10">
          ...
          return (
            <Link
              ...
              className={`group relative flex items-center gap-2 whitespace-nowrap px-4 py-3.5 text-[13px] font-medium transition-colors ${
                active ? "text-charcoal" : "text-ink-faint hover:text-charcoal"
              }`}
            >
              <Icon size={15} className={active ? "text-gold-dim" : "text-ink-faint group-hover:text-ink-soft transition-colors"} />
              {label}
              <span
