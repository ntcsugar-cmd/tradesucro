  ...
  const columns: DataTableColumn<Purchase>[] = [
    {
      ...
      render: (p) => (
        <div>
          <p className="font-mono text-xs text-charcoal">{p.purchaseNumber}</p>
          <p className="text-[11px] text-ink-faint mt-0.5">{new Date(p.purchaseDate).toLocaleDateString("en-IN", { day: "2-digit", month: "short" })}</p>
        </div>
      ),
    },
    {
      ...
      render: (p) => (
        <div>
          <p className="font-medium text-charcoal">{p.supplier}</p>
          {p.supplier !== p.mill && <p className="text-[11px] text-ink-faint mt-0.5">via {p.mill}</p>}
        </div>
      ),
    },
    { key: "broker", header: "Broker", render: (p) => (p.broker ? p.broker : <span className="text-ink-faint">—</span>) },
    ...
    { key: "totalCost", header: "Total Amount", align: "right", render: (p) => <span className="font-mono font-medium text-charcoal">{formatINR(p.totalCost)}</span> },
    {
      key: "dealReference",
      header: "Deal Reference",
      render: (p) => (p.dealReference ? <span className="font-mono text-xs text-gold-dim">{p.dealReference}</span> : <span className="text-ink-faint">—</span>),
    },
